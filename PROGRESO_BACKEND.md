...
- Hay backend real parcial para consorcios, gastos, expensas y pagos.
...
| 4 | Consorcios | Prioridad 1 | Parcial | `Controllers/ConsorciosController.cs` lista, muestra detalle y crea consorcios con unidades funcionales; valida números de UF duplicados; existen vistas `Index`, `Create` y `Details`. | Implementar edición real; crear `ConsorcioVM`/VMs de UF; mejorar validaciones de email, DNI, CUIT y cantidad de pisos; probar persistencia completa. | Terminar `Edit` y formalizar ViewModels de consorcio/unidades. |
| 5 | Unidades funcionales | Prioridad 1 | Parcial | `Models/UnidadFuncional.cs`; relación con `Consorcio`; alta de UF dentro de `POST Consorcios/Create`. | Falta CRUD o edición específica de UF; asociación fuerte con `Usuario` propietario; validaciones completas; actualización posterior de datos de propietario. | Implementar edición de UF desde el detalle del consorcio. |
| 6 | Gastos | Prioridad 2 | Parcial | `Models/Gasto.cs`, `Controllers/GastosController.cs`, `Services/GastoService.cs`, `Services/FileStorageService.cs`, `ViewModels/GastoVM.cs`; `IGastoService` e `IFileStorageService` registrados en `Program.cs`; índice único `NumeroFactura`; upload a `wwwroot/uploads`. | Faltan filtros completos para propietario; faltan vistas `Edit`/`Delete` o ajuste de flujo; falta autorización por rol. | Probar `/Gastos`, `/Gastos/Create`, `/Gastos/MisGastos` en runtime y luego completar filtros/autorización. |
...
- Backend de gastos: estructura de modelo, controlador, servicio, ViewModel, almacenamiento de archivos y registro DI en `Program.cs`.
...
Los dos ajustes técnicos mínimos quedaron cerrados: servicios de gastos registrados y factory de EF preparada para archivos opcionales o variable de entorno.

El módulo lógico para atacar ahora es `Consorcios + Unidades funcionales`, porque es Prioridad 1 en la planificación y todavía falta completar edición, ViewModels y validaciones formales.
