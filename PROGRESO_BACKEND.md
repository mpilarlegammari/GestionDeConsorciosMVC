...
| 5 | Unidades funcionales | Prioridad 1 | Hecho | `UnidadFuncionalViewModel`; alta de UFs dentro de `POST Consorcios/Create`; edición de UFs existentes desde `POST Consorcios/Edit`; validaciones de UF requerida, DNI requerido, email válido y duplicados por consorcio; `IUsuariosService` sincroniza usuarios propietarios desde UFs con mail y DNI. | Queda para una etapa posterior evaluar relación formal `UsuarioId` e índice único compuesto si se decide tocar migraciones. | Continuar usando `MailPropietario` como vínculo funcional entre UF y usuario propietario para la entrega académica. |
...
| 7 | Expensas | Prioridad 3 y 5 | Parcial | Admin reforzado: `IExpensasService`, `ExpensasService`, `ExpensasIndexViewModel`, `GenerarExpensasViewModel`, `ExpensaDetailsViewModel`; `Index` con filtros reales; generación desde gastos del período; validación de duplicados por consorcio/período; detalle sin `ViewBag`; propietario reforzado: `MisExpensasViewModel`, `MisExpensas` usa `UserEmail`, UFs propias, pagos incluidos, filtros por UF/período/año/estado, totales pendientes/pagados y retorno a detalle con filtros; build en `0 errores`. | Falta PDF/liquidación, autorización real y criterios de distribución más avanzados. | Próxima tanda posible: reforzar `MisPagos` con ViewModel y filtros, o mejorar seed/testing. |
| 8 | Pagos | Prioridad 4 | Parcial | Admin reforzado: `IPagosService`, `PagosService`, `PagosAdminIndexViewModel`, `PagoDetailsViewModel`, `RevisarPagoViewModel`; `Index` admin con filtros por estado/consorcio/período; detalle tipado; aprobación cambia `Pago.Estado` a `Aprobado` y `Expensa.Estado` a `Pagada`; rechazo cambia `Pago.Estado` a `Rechazado`; propietario reforzado: `MisPagosViewModel`, `MisPagos` usa `UserEmail`, UFs propias, filtros por UF/período/año/estado/medio, totales por estado y retorno a detalle con filtros; build en `0 errores`. | Falta autorización real por rol y eventualmente mover `InformarPago` a servicio si se busca homogeneidad completa. | Próxima tanda posible: mejorar seed/testing o avanzar sobre autenticación básica. |
...
| 12 | Login/autenticación | Alta | Parcial | `Controllers/AuthController.cs` valida email, contraseña en texto plano, rol y estado contra `Usuarios`; para propietario exige UF asociada por `MailPropietario`; `Program.cs` registra sesión y `IUsuariosService`; `DevDataSeeder` asegura admin demo `admin@admin.com` en Development. | Falta password hash real, cookies/claims, `[Authorize]` y protección formal de rutas por rol. | Probar login admin y propietario con datos seed/UF; dejar autorización formal para una tanda separada si el TP lo requiere. |
...
El próximo foco lógico es mejorar seed/testing para una demo con datos reales o avanzar sobre autenticación básica, dejando limpieza técnica de migraciones/archivos heredados para una tanda separada.
